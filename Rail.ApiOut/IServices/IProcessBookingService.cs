...
namespace Rail.ApiOut.IServices
{
    public interface IProcessBookingService
    {
        ...
        Task<BookingModel> GetBookings(string correlationId);
        ...
        Task<(List<BookingItemsModel> ticketItems, string result, List<KeyValuePair<string, string>>, List<decimal> totalAgentAmount)>
            HandleElseCondition(SearchHistoryModel searchResult, List<string> offerLocations, string item, string _correlation, string _Currency, string _AgentID);
